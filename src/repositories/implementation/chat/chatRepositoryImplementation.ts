...
//     try {
//         return await Chat.findById(chatId).populate('participants.participantId')
...
async getChatsByParticipant(participantId: string): Promise<IChat[]> {
  try {
    const chats = await Chat.find({ 'participants.participantId': participantId })
      .sort({ lastActivity: -1 })
      .populate({
        path: 'participants.participantId',
    ...
    for (const chat of chats) {
      for (const participant of chat.participants) {
        if (participant.role === 'user') {
          const user = await User.findById(participant.participantId).select('fullName profilePhoto');
          if (user) {
            participant.name = user.fullName;
            participant.profilePhoto = user.profilePhoto;
          } else {
            console.warn(`User with ID ${participant.participantId} not found`);
          }
        } else if (participant.role === 'tutor') {
          const tutor = await Tutor.findById(participant.participantId).select('name profilePhoto');
          if (tutor) {
            participant.name = tutor.name;
            participant.profilePhoto = tutor.profilePhoto;
          } else {
            console.warn(`Tutor with ID ${participant.participantId} not found`);
          }
        }
      }
    }
